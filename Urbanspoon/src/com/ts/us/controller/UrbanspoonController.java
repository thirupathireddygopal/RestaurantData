		...
		try {
			if (isMultipart) {
				...
				if (action.equals("login")) {
					...
					String loginAs = request.getParameter("loginAs"); // required values are "user","restaurant"
					
					if (loginAs.equals("user")) {
						if (UrbanspoonHelper.loginAsUser(request, response)) { // return true and storing the value of [user_id] in HttpSession
							List<Restaurant> restaurantsList = UrbanspoonHelper.getRestaurants(true);
							request.setAttribute("restaurantsList", restaurantsList);
							request.getRequestDispatcher("userHome.jsp").forward(request, response); // dispatching to userHome.jsp
						}
				...
				else if (action.equals("branch_feedback")) { // giving the feedback for branch by providing restaurant_id and branch_id
